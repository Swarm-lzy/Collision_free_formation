function [error, Phi, Wd, M] = mntCtrl(param, X, Xd, Bd)
...
% Desired R
A = -param.kp*ex - param.kv*ev - param.m*param.g*param.e3 + param.m*xd_2dot;
ea = param.g*param.e3 + dot(A, R*param.e3)*R*param.e3/param.m - xd_2dot;
A_dot = -param.kp*ev - param.kv*ea + param.m*xd_3dot;
...
A_2dot = -param.kp*ea - param.kv*ej + param.m*xd_4dot;
...
% Attitude errors
eR = vee_map(Rd'*R - R'*Rd)/2;

Wd = vee_map(Rd'*Rd_dot);
Wd_dot = vee_map(Rd'*Rd_2dot - hat_map(Wd)^2);
eW = W - R'*Rd*Wd;

% Attitude control for UAV
M = -param.kr*eR - param.kw*eW + cross(W, param.J*W)...
    - param.J*(hat_map(W)*R'*Rd*Wd - R'*Rd*Wd_dot);
